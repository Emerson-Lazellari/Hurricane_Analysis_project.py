...
MILL_CONVERSION = 1000000
BILL_CONVERSION = 1000000000
damages_updated = []


def updated_recorded_damages(data):
    for data in damages:
        if "M" in data:
            new_data_mill = data.strip("M")
            new_data_mill_float = float(new_data_mill)
            damages_updated.append(new_data_mill_float * MILL_CONVERSION)
        elif "B" in data:
            new_data_bill = data.strip("B")
            new_data_bill_float = float(new_data_bill)
            damages_updated.append(new_data_bill_float * BILL_CONVERSION)
        else:
            damages_updated.append(data)


# test function by updating damages
updated_recorded_damages(damages)
